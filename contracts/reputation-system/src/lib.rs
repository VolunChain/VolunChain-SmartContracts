...
impl ReputationSystem {
    ...
    pub fn add_organization(env: Env, admin: Address, org: Address) {
        ...
        let mut organizations: Vec<Address> = env.storage().instance().get(&DataKey::Organizations).unwrap();
    ...
    pub fn endorse_volunteer(
        ...
        let organizations: Vec<Address> = env.storage().instance().get(&DataKey::Organizations).unwrap();
        ...
        let mut endorsements: Map<Address, u32> = env.storage().instance()
            .get(&endorsement_key)
    ...
    fn update_reputation(env: &Env, volunteer: &Address) {
        ...
        // Sum endorsements
        if let Some(endorsements) = env.storage().instance().get(&DataKey::Endorsements(volunteer.clone())) {
        ...
        if let Some(badges) = env.storage().instance().get(&DataKey::Badges(volunteer.clone())) {
            let badges: Vec<Symbol> = badges;
            total_score += badges.len() as u32 * 10; // Each badge adds 10 points
        }

        // Store updated reputation
        env.storage().instance().set(&DataKey::Reputation(volunteer.clone()), &total_score);
    }
